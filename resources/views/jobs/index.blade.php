    ...
    <div class="mb-4 flex items-center justify-between">
        <div class="flex items-center gap-2">
            ...
            @if (!isset($tag) && !isset($employer))
                <!-- auth () -->
                /
                <h3 class="text-3xl"><a href="{{ route('jobs.create') }}" class="text-blue-600 hover:underline">Create</a>
                </h3>
            @endif
            ...
            {{-- <style> --}}
            {{--     .clr { --}}
            {{--         background-color: rgb(from rgb(255, 255, 60) 208 228 228); --}}
            {{--     } --}}
            {{-- </style> --}}
            {{-- --}}
            {{-- <div class="clr">BOOM</div> --}}
                                ...
                                <dd class="text-md my-1 border-b border-t border-gray-400 pb-1 font-bold">
                                    ...
                                    @else
                                        <a class="text-blue-500 hover:underline"
                                            href="{{ route('jobs.employer', $job->employer->id) }}">
                                            {{ $job->employer->name }}
                                        </a>
                                    @endisset
                                </dd>
                                ...
                                @if ($job->tags)
                                    <dd class="italic">
                                        @foreach ($job->tags as $tag)
                                            <a href="{{ route('jobs.tag', $tag->name) }}"
                                                class="mx-1 text-blue-400 hover:underline">
                                                {{ $tag->name }}
                                            </a>
                                            @if (!$loop->last)
                                                ,
